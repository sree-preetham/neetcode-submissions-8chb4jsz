...
class Solution {
    ...
    private int gridBfs(int[][] grid, int row, int col, ArrayDeque<Cell> queue){
    ...
    private int gridDfs(int[][] grid, int row, int col){
        grid[row][col] = -1;
        int[] dirX = new int[]{-1,1,0,0};
        int[] dirY = new int[]{0,0,-1,1};
        int ans = 0;
        for(int i = 0; i < 4; i++){
            ...
            int ncol = col + dirY[i];
            if(check(nrow, ncol, grid)){
                ans = ans + gridDfs(grid, nrow, ncol);
            }
        }
        return 1 + ans;
    }
    public int maxAreaOfIsland(int[][] grid) {
        ...
        for(int i = 0; i < grid.length; i++){
            for(int j = 0; j < grid[i].length; j++){
                if(grid[i][j] == 1){
                    ans = Math.max(ans, gridBfs(grid, i, j, queue));
                    // ans = Math.max(ans, gridDfs(grid, i, j));
                }
            }
        }
        return ans;
