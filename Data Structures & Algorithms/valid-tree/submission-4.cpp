...
public:
    ...
    bool dfsOptimized(int src, vector<vector<int>>& adj, vector<int> &vis, int parent){
        vis[src] = true;
        for(int &neighbour: adj[src]){
            if (!vis[neighbour]) {
                if(dfsOptimized(neighbour, adj, vis, src)) return true;
            }
            else if(neighbour!=parent) return true;
        }
        return false;
    }
    bool bfsCheckCycle(int src, vector<vector<int>>& adj, vector<int> &vis){
    ...
    bool validTree(int n, vector<vector<int>>& edges) {
        ...
        for(int i = 0; i < n; i++){
            if(!vis[i]) {
                ...
                // ans = !dfsOptimized(i, adj, vis, -1);
                ans = !bfsCheckCycle(i, adj, vis);
                component++;
            }
        }
